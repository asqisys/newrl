...
#				block = chain[0];
			...
			transactions=[{'timestamp': timestamp, 'type': 0, 'currency': "INR", 'fee': 0.0, 'descr': "Genesis Block", 'valid':1, 'block_index': 0, 'specific_data':[]}]
			block = {'index': 1,
				'timestamp': timestamp,
				'proof': 0,
...
#		self.create_block(block['timestamp'],block['proof'], 0, block['text'])
	...
	def get_last_block_index(self):
		last_block_cursor = self.cur.execute(f'''SELECT block_index FROM blocks ORDER BY block_index DESC LIMIT 1''')
...
#		print("previous_hash = ",previous_hash)
		block = self.create_block(block['timestamp'], proof, previous_hash, text)
...
#	        newchain = Blockchain(chainfile)	#chainfile also includes a genesis block so that's what we use to init
		...
		if self.chain_valid(self.chain):
			...
			print("Tried to load chain from ",chainfile,", but the chain is invalid.")
			return False
#			exit;
...
#textarray=[];
#for file in files:
#	if "create.json" in file:
#		with open(file, "r") as read_file:
#			print("processing ",file)
#			transaction=json.load(read_file)
#		ts = datetime.datetime.strptime(transaction['timestamp'][:-7],"%Y-%m-%d %H:%M:%S");
#		print(ts)
#		if ts > latest_ts:
...
#				print("Found valid transaction, adding to block")
...
#else:
#	print("No new transactions, leaving chain untouched")
