...
#					"wallet_address":[],
	...
	def getvalidadds(self):
		...
		if ttype==5:    # one way transfer; only sender1 is needed to sign
			validadds.append(trans['specific_data']['wallet1'])
		if ttype==6:    # lending transactions; lender, borrower and security provider need to sign
			validadds.append(trans['specific_data']['borrowerwallet'])									
			for lender in trans['specific_data']['lenders']:        #lenders is a list of dicts, has at least one address
				validadds.append(lender['wallet'])              # format for each lender is {'wallet':0xsdswe,'amount':1220}
			for secprovider in trans['specific_data']['secproviders']:  # secproviders is a list of dicts, can have zero addresses
				validadds.append(secprovider['wallet'])         # format for each secprovider is {'wallet':0xsdswe,'amount':1220,...}
		return validadds
...
#	def techvalidator(self):
	...
	#	if not mempool:
		...
		#	self.loadtransaction
			...
			if ttype==6:	#lending so lenders will need to treat their lending as outflows and secproviders of their security tokens
				for lender in trandata['specific_data']['lenders']:
					if sender==lender['wallet'] and trandata['specific_data']['tokencode']==tokencode:
						mppayment+=lender['amount'];
				for secprovider in trandata['specific_data']['secproviders']:
					if sender==secprovider['wallet'] and trandata['specific_data']['sec_token_code']==tokencode:
						mppayment+=secprovider['amount'];
