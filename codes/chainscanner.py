...
class Chainscanner():
    def __init__(self, chainfile=CHAIN_FILE):
...
#        return len(blockchain.chain)
    ...
    def gettransactions(self, blockindex):
        ...
        transactions = self.blockchain.chain[blockindex -
                                             1]['text']['transactions']
        return transactions
...
#            transactions=self.gettransactions(index)
        for wallet in wallets:
            ...
            balances.append(
                {'wallet_address': wallet['wallet_address'], 'balance': balance})
        return balances
    ...
    def getalltransids(self):
        ...
        while index < self.chainlength+1:
            ...
            for transaction in transactions:
                try:
                    tranids.append(
                        {"trans_code": transaction['trans_code'], "blockindex": blindex})
                except:
                    continue
        return tranids
...
#        for idx in (1,self.chainlength+1):    #blockindex starts at 1 and not 0
    ...
    def getallbalances(self):
        ...
        for wallet in wallets:
            #            tokenbal={}
            for token in tokens:
                balance = self.getbaladdtoken(
                    wallet['wallet_address'], token['tokencode'])
#                tokenbal.update({token,balance})
                balances.append(
                    {'wallet_address': wallet['wallet_address'], 'tokencode': token['tokencode'], 'balance': balance})
                print({'wallet_address': wallet['wallet_address'],
                      'tokencode': token['tokencode'], 'balance': balance})
        return balances
                ...
                if transaction['type'] == 2:
                    ...
                    if transaction['specific_data']['first_owner'] == address:
                        if transaction['specific_data']['tokencode'] == tokencode:
                            ...
                            balance = balance + \
                                transaction['specific_data']['amount_created']
                # this is a asset transfer transaction
                if transaction['type'] == 4 or transaction['type'] == 5:
                    if transaction['specific_data']['wallet1'] == address:
                        # wallet1 is sending
                        if transaction['specific_data']['asset1_code'] == tokencode:
                            balance = balance - \
                                transaction['specific_data']['asset1_number']
                        # wallet1 is recvng
                        if transaction['specific_data']['asset2_code'] == tokencode:
                            balance = balance + \
                                transaction['specific_data']['asset2_number']
