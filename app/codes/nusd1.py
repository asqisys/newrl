...
class nusd1():
    ...
    def __init__(self, contractaddress=None):
        self.template = "nusd"
        self.version = "1.0.0"
        # this is for instances of this class created for tx creation and other non-chain work
        self.address = contractaddress
        if contractaddress:  # as in this is an existing contract
            con = sqlite3.connect(NEWRL_DB)
            cur = con.cursor()
            # this will populate the params for a given instance of the contract
            self.loadcontract(cur, contractaddress)
            con.close()
    ...
    def setup(self, cur, callparamsjson):
        ...
        self.address = '0x' + keccak_digest[-40:]
        ...
        contractparams['ts_init'] = time.mktime(
        ...
        sdestr = 0 if not contractparams['selfdestruct'] else int(
            contractparams['selfdestruct'])
        cstatus = 0 if not contractparams['status'] else int(
...
#    def create_tx(self,ttype,tspdata,currency="INR",fee=0.0,descr=None):
    ...
    def deploy(self, cur, sender, callparams):
        ...
        # the contract is not setup, i.e. is either yet to be setup, already deployed or terminated
        if self.contractparams['status'] != 1:
            print(
                "The contract is not in the post-setup stage. Exiting without deploying.")
            return False
        else:
            if self.sendervalid(sender, self.deploy.__name__):
                # changed from 1 (setup done) to 2 (deployed and live)
                self.contractparams['status'] = 2
                cur.execute(f'''INSERT OR REPLACE INTO contracts
    		    		(address, status)
	    		    	VALUES (?, ?)''', (self.address, self.contractparams['status']))
                print("Deployed smart contract - ", self.template,
                      "with address ", self.contractaddress)
    ...
    def transferlend(self):
        transferdata = {"asset1_code": self.contractparams['contractspecs']['tokencode'],
                        ...
                        "asset2_number": 0}
        # going with default values for mempool="./mempool",statefile="./state.json",descr=None
        tx = self.create_tx(5, transferdata, "INR", 0.0)
        return tx
    ...
    def reverseandterminate(self):
        ...
        if self.contractparams['status'] == 0:
            print(
                "This contract is yet to be setup, nothing is expected to be in its wallet. Exiting.")
            return False
        ...
        #########
        self.terminate()
        print("Reversed and terminated smart contract - ",
              self.template, "with address ", self.contractaddress)
        return True

    def terminate(self):
        if self.contractparams['status'] == 2:
            print(
                "This is a deployed contract, cannot terminate it while it is live. Exiting.")
            return False
        # changed from 1 (setup done) to -1 (terminated)
    ...
    def close(self):
        ...
        if self.contractparams['status'] == 3:
            ...
            flag = self.terminate()
            if flag:
                print("Contract ", self.contractaddress,
                      " has been terminated.")
                return True
            else:
